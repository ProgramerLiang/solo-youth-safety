...
| 前端 / Android 版本 | `0.8.5` | `frontend/package.json` |
| 本地快照导出版本 | 跟随 `frontend/package.json` 输出当前版本 | `frontend/src/data/snapshot.ts` |
| package-lock 版本元数据 | 已收口为 `0.8.5` | `frontend/package-lock.json` |
...
- 导航：底栏 4 Tab 为主导航（首页、场景面板(含事件消息)、AI、我的面板(含会员)），侧边栏仅保留辅助页（SOS/历史/回放/轨迹）。底栏支持小屏横向滑动，活跃 Tab 带缩放弹跳动画。页面间支持一键跳转（SOS → 配置、首页卡片 → 对应页、场景面板 → 围栏锚点）。
