...
| 前端 / Android 版本 | `0.4.33` | `frontend/package.json` |
| 本地快照导出版本 | 跟随 `frontend/package.json` 输出当前版本 | `frontend/src/data/snapshot.ts` |
| package-lock 版本元数据 | 已收口为 `0.4.33` | `frontend/package-lock.json` |
...
- 页面：总览、SOS、历史、行程历史、回放、轨迹、配置、联系人、主题、工具。
- 导航：左上角按钮打开抽屉导航；页面切换仅保留侧边栏入口，不再保留顶部页面跳转条或总览页快捷导航块。
- 总览：安全行程卡片前置，风险等级色块、风险分组折叠卡片、本地行程统计面板和本地-only 声明已接入。
...
- SOS 模拟训练：SOS 页可启动 5 秒倒计时模拟训练，不拨打电话、不发短信、不写历史，结果步骤标注"训练模式"，可重新训练或返回。
- 历史：本地 SOS 结果列表与步骤详情。
...
- 行程历史：侧边栏可进入安全行程历史页，读取本地最多 50 条归档行程，展示目的地、状态、预计到达时间和事件数；不导出、不上报。
- 行程历史详情：点击行程历史条目展开事件时间线，展示创建 / 延长 / 到达 / 取消 / 超时记录及时间戳。
- 行程预设：配置页可管理常用行程预设（目的地 + 预设时长），创建行程时可快速选用预设。
- 本地通知：配置页可关闭总开关、行程超时提醒和风险变化提醒；行程提醒可配置提前 1 / 5 / 10 / 15 分钟；通知仅为本机提醒，不承诺后台、熄屏或 force-stop 后送达，不自动报警。
- 行程统计：总览页展示累计行程数、平均时长、准时率和高频目的地排行，基于本地归档行程计算。
- 主题：明暗模式、预设色、自定义色、Android 动态色接口尝试；选中态 Chip 使用主题对比色处理悬停态。
- 隐私 PIN 锁：配置页可启用 PIN 锁，设置 4 位数字 PIN；启用后应用启动需验证 PIN，本地存储 PIN 哈希值，不远端上报。
...
4. **工程一致性修复**
   ...
   - `frontend/package-lock.json` 顶层版本元数据已同步到 `0.4.33`。
5. **回归基线固定**
...
- **本地通知 + 总览仪表盘打磨 ✅** — 配置页可控制本地通知；安全行程可调度本机行程提醒，总览风险升级可调度本机风险提醒并做 30 分钟防抖；总览页新增风险等级色块、分组风险卡片和本地-only 声明。仍不做远程推送、自动 SOS、后台保活或 force-stop 后通知承诺。
- **安全行程历史 + 轨迹回放增强 ✅** — 新增安全行程历史页，复用本地归档行程展示状态、时间和事件数；回放页新增缩放控制、点位详情、轨迹点速度着色和速度图例。仍只读取本机历史数据，不接远端、不实时监护、不替换地图 SDK。
- **SOS 模拟训练 + 行程历史详情 + 回放动画播放 ✅** — SOS 页新增模拟训练模式，5 秒倒计时后展示训练结果，不触发真实动作；行程历史列表条目可展开事件时间线；回放页新增播放 / 暂停、1x / 2x / 4x 倍速和逐点高亮动画。仍不自动触发 SOS、不接远端、不替换地图 SDK。
- **行程预设管理 + 行程统计面板 + 隐私 PIN 锁 ✅** — 配置页新增行程预设管理，可保存常用目的地与时长；总览页新增行程统计面板，展示累计行程数、平均时长、准时率和高频目的地；配置页新增隐私 PIN 锁开关与 PIN 设置，启用后应用启动需 4 位数字 PIN 验证。仍只本地存储，不远端上报或同步。
...
- `npm test` 覆盖短信模板、SOS 状态归一、轨迹队列、位置新鲜度 hook、SOS 倒计时 hook、原生桥结果映射、系统定位非法坐标回退、轨迹存活期定时采样 / 持久化 / 本地确认、快照版本对齐、侧边栏导航约束、行程历史页、配置页版本与能力边界事实面板、配置页短信预览、本地通知配置 / 调度 / 防抖、总览风险等级与分组仪表盘、轨迹回放缩放 / 点位详情 / 速度图例、全局填充按钮与主题页选中态 Chip 主题对比度。
