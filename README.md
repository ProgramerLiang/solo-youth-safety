...
| 前端 / Android 版本 | `0.4.29` | `frontend/package.json` |
| 本地快照导出版本 | 跟随 `frontend/package.json` 输出当前版本 | `frontend/src/data/snapshot.ts` |
| package-lock 版本元数据 | 已收口为 `0.4.29` | `frontend/package-lock.json` |
...
4. **工程一致性修复**
   ...
   - `frontend/package-lock.json` 顶层版本元数据已同步到 `0.4.29`。
5. **回归基线固定**
   - 将 SOS 状态机、Android 能力边界文案、轨迹队列语义、原生桥错误归一、系统定位非法坐标回退、Android 快速低精度优先定位、定位链路自检、首次权限配置引导、本地风险规则配置、本地安全行程倒计时、诊断报告脱敏导出、诊断摘要解析、导出目录提示、地理围栏窄屏可触达布局纳入稳定回归范围。
...
- **本地安全行程 / 守护倒计时 ✅** — 总览页可创建短时本地安全行程，展示预计到达时间，支持手动确认到达、延长和取消；行程超时未确认会进入本地风险提示和脱敏诊断摘要。仍不自动触发 SOS、短信、电话或远端上报，也不承诺后台 / force-stop 后运行。
